...
public class pleasetell {
		...
		public List<Double> freqVisualizer(){
			...
			try {
				...
				AudioSample jingle = minim.loadSample("d:\\BBk4PadVerb105A-01.mp3", 2048);

				
				float[] leftChannel = jingle.getChannel(AudioSample.LEFT);
				System.out.println("channel length: " + leftChannel.length);
				System.out.println("length / 500: " + (leftChannel.length / 15) );
				System.out.println("near pow2: " + (tempPowerOfTwo(leftChannel.length / 15) ));
				...
				int fftSize = (int)(tempPowerOfTwo(leftChannel.length/30));
